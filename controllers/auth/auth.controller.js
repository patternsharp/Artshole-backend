...
exports.Welcome = async (req, res) => {
  try {
    ...
    if (!authorization) {
      return res
        .status(401)
        .send({ status: false, message: "Authorization token missing" });
    }
  ...
  try {
    ...
    if (remember == true) {
      accessToken = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, {
        expiresIn: process.env.JWT_EXPIRES_IN_remember,
      });
    } else {
      accessToken = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, {
...
exports.SignInWithGoogle = async (req, res) => {
  try {
    ...
    let accessToken = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, {
      expiresIn: process.env.JWT_EXPIRES_IN_remember,
    });
  ...
  try {
    ...
    let update = {
      ...
      avatarUrl: req.files?.avatarUrl
        ? req.files.avatarUrl[0].filename
        : avatarUrl,
...
exports.UpdateSocial = async (req, res) => {
  try {
    const { _id, facebookLink, instagramLink, linkedinLink, twitterLink } =
      req.body;
