...
exports.AddArtworkProperty = async (req, res) => {
    ...
    try {
        ...
        if (art_Property) {
            return res.status(400).send({ status: false, message: "This Property item already exists" });
        }
        ...
        await art_Property.save((error) => {
            ...
            return res.status(200).send({ status: true, message: "Added successfully.", result: art_Property });
        });
    } catch (error) {
        console.error(error);
        return res.status(500).send({ status: false, message: 'Internal server error' });
...
exports.UpdateArtworkProperty = async (req, res) => {
    ...
    try {
        ...
        ArtworkProperty.findByIdAndUpdate(itemId, { itemTitle: itemTitle }, (error) => {
            if (error) {
                return res.status(500).send({ status: false, message: 'Internal server error' });
            }
...
exports.DeleteArtworkProperty = async (req, res) => {
    ...
    try {
        if (req.body && req.body.itemId) {
            const response = await ArtworkProperty.deleteOne({ _id: req.body.itemId });
