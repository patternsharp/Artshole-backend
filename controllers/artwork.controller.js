...
exports.AddArtwork = async (req, res) => {
    try {
        let {
            artworkTitle, userId, artworkImg, description, price, width, height, depth, status, category, year, propertyList
        } = req.body;
        ...
        if (art) {
            return res.status(400).send({ status: false, message: "This Artwork already exists" });
        }
        ...
        return res.status(200).send({ status: true, message: "Added successfully.", result: art });
    } catch (error) {
        console.error(error);
...
exports.UpdateArtwork = async (req, res) => {
    try {
        let {
            _id, artworkTitle, userId, artworkImg, description, price, width, height, depth, status, category, year, propertyList
        } = req.body;

        console.log("Update artwork req.body=>", req.body)
        console.log("Update artwork req.files=>", req.files)

        let art = await Artwork.findOne({ _id: _id, isDeleted: false, isBlocked: false });
        ...
        art = await Artwork.findOne({ _id: _id })
            ...
            .exec();
        return res.status(200).send({ status: true, message: "Updated successfully.", result: art });
    } catch (error) {
        console.error(error);
        return res.status(500).send({ status: false, message: 'Internal server error' });
    }
...
exports.DeleteArtwork = async (req, res) => {
    try {
        ...
        if (!art) {
            return res.status(400).send({ status: false, message: 'This artwork has already been deleted.' });
        }
        ...
        if (searchObject) {
            ...
            if (property.length > 0) {
                if (category == 'All') {
                    ...
                    if (searchText == null) {
                        query = {
                            isDeleted: false, isBlocked: false, category: category,
                            price: { $gte: priceRange[0], $lte: priceRange[1] },
                            propertyList: { $in: property }
                        }
                    } else {
                ...
                if (category == 'All') {
                    ...
                    if (searchText == null) {
                        if (priceRange[0] == 0 && priceRange[1] == 10000) {
                            query = { isDeleted: false, isBlocked: false, category: category }
                        } else {
                            query = {
                                isDeleted: false, isBlocked: false, category: category,
                                price: { $gte: priceRange[0], $lte: priceRange[1] }
