...
exports.AddVideo = async (req, res) => {
    try {
        ...
        let oneVideo = await Video.findOne({ videoLink: videoLink, isDeleted: false });

        if (oneVideo) {
            if (oneVideo.isBlocked == false) {
                return res.status(400).send({ status: false, message: "This video already exists" });
            } else if (oneVideo.isBlocked == true) {
                return res.status(400).send({ status: false, message: "This is a banned video" });
            }
        }
        ...
        return res.status(200).send({ status: true, message: "Added successfully.", result: oneVideo });
    } catch (error) {
        console.error(error);
...
exports.UpdateVideo = async (req, res) => {
    try {
        ...
        return res.status(200).send({ status: true, message: "Updated successfully.", result: video });
    } catch (error) {
        console.error(error);
        return res.status(500).send({ status: false, message: 'Internal server error' });
    }
...
exports.DeleteVideo = async (req, res) => {
    try {
        ...
        if (!video) {
            return res.status(400).send({ status: false, message: 'This video has already been deleted.' });
        }
...
exports.GetClientVideos = async (req, res) => {
    try {
        ...
        if (!video_List) {
            return res.status(400).send({ status: false, message: 'There is no video.' });
        }
