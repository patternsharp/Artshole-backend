...
exports.AddNotification = async (req, res) => {
    try {
        ...
        return res.status(200).send({ status: true, message: "Added successfully.", result: notification });
...
exports.GetNotifications = async (req, res) => {
    try {
        ...
        let notification_List = await Notification.find({ isDeleted: false, toUser: userId })
            .sort({ createdAt: -1 })
            .populate("toUser")
            .populate("fromUser")
            .exec();
...
exports.ReadNotifications = async (req, res) => {
    try {
        const { userId } = req.body
        let notification_List = await Notification.find({ isDeleted: false, toUser: userId }).updateMany({}, { $set: { isUnRead: false } })
...
exports.DeleteNotifications = async (req, res) => {
    try {
        ...
        itemIds.map(async (e) => {
            ...
            let doc = await Notification.findOneAndUpdate(filter, update, { new: true });
        })
