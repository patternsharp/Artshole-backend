...
exports.AddCollection = async (req, res) => {
    try {
        ...
        let collection = await Collection.findOne({ collectionTitle: collectionTitle });

        if (collection) {
            return res.status(400).send({ status: false, message: "This Collection already exists" });
        }
        ...
        return res.status(200).send({ status: true, message: "Added successfully.", result: collection });
    } catch (error) {
        console.error(error);
...
exports.DeleteCollection = async (req, res) => {
    try {
        ...
        if (!collection) {
            return res.status(400).send({ status: false, message: 'This collection has already been deleted.' });
        }
...
exports.GetClientCollections = async (req, res) => {
    try {
        let collection_List = await Collection.find({ isDeleted: false, isBlocked: false })
            .populate({
                path: 'author',
                populate: [
                    { path: 'jobCategory' },
...
exports.GetCollectionDetailsById = async (req, res) => {
    try {
        let oneCollection = await Collection.find({ isDeleted: false, isBlocked: false, _id: req.body.collectionId })
            .populate({
                path: 'author',
                populate: [
                    { path: 'jobCategory' },
...
exports.SearchCollections = async (req, res) => {
    ...
    try {
        if (req.query.keyword) {
            ...
            return res.status(200).send({ status: true, collectionList: collections });
        }
    } catch (err) {
        console.error(err.message);
        return res.status(500).send({ status: false, message: 'Internal server error' });
...
exports.GetMyCollections = async (req, res) => {
    try {
        ...
        let collection_List = await Collection.find({ isDeleted: false, isBlocked: false, author: userId })
            .populate({
                path: 'author',
                populate: [
                    { path: 'jobCategory' },
...
exports.UpdateArtworkOnCollection = async (req, res) => {
    try {
        ...
        if (!flag) {
            ...
            await collection.save()
            return res.status(200).send({ status: true, message: 'success', result: collection });
        } else {
